        ...
        if self.writeDataOutput:
            ...
            if self.newOutDataFNm is None:
                ...
                for fNm,vNms in outputFNmsAndVars.items():
                    ...
                    newCmd['params']['OutFileName'] = self.outDataFNm
...
def UsageDie():
    print '{} InScriptNm:MPilot20FName OutScriptNm:MPilot30FName [InDataNm:InFileNm.nc OutDataNm:OutFileNm.nc WriteDataOutput:TrueOrFalse]'.format(os.path.basename(sys.argv[0]))
    print
    print 'Convert MPilot 2.0 script to MPilot 3.0 script\n'
    
    exit()
# def UsageDie():

if len(sys.argv) < 3: UsageDie()

# Set the defaults for the optional args
...
for argv in sys.argv[1:]:
    argNm,argVal = argv.split(':',2)
    args[argNm] = argVal

if args['WriteDataOutput'] == 'True':
    args['WriteDataOutput'] = True
else:
    args['WriteDataOutput'] = False

# Instantiate the class object
cv = Converter(
    args['InScriptNm'],
    args['OutScriptNm'],
    newInDataFNm=args['InDataNm'],
    newOutDataFNm=args['OutDataNm'],
    writeDataOutput=args['WriteDataOutput']
    )
